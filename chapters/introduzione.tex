...
Ogni progetto viene svolto in tutte le sue parti dal \textit{team} di Zero12, dalla fase di progettazione alla fase di sviluppo. \\
...
La metodologia adottata dall'azienda è quella \textit{Agile} ada in particolare al \textit{framework Scrum}.\\ 
Ogni progetto viene suddiviso in \textit{sprint} di durata fissa (durante il mio periodo di \textit{stage} la durata era di una settimana) e viene assegnato un \textit{team} di lavoro (il \textit{team} di lavoro era composto dal mio \textit{tutor} aziendale come \textit{project manager} e da me come \textit{software developer}).
\begin{figure}[H]
    \centering
...
\subsubsection{Jira}
\textit{Jira} è un \gls{itsg} che permette la gestione delle \textit{task} dei progetti in modo \textit{Agile}. All'inizio di ogni \textit{sprint}, lo \textit{scrum master} crea una nuova \textit{board} relativa allo \textit{sprint} in corso e assegna le \textit{task} da svolgere ai membri del \textit{team} di lavoro.
I membri del \textit{team} di lavoro possono notificare lo stato di avanzamento delle \textit{task}, spostandole tra le varie colonne quali: 
\begin{itemize}
...
Permette la creazione di canali di comunicazione dedicati a specifici argomenti. In ambito aziendale, viene creato un canale dedicato ad ogni progetto in modo da poter permettere una comunicazione più efficace tra i membri del \textit{team} di lavoro.
Nel caso del mio progetto di \textit{stage} è stato creato un canale dedicato al progetto assieme al mio \textit{tutor} aziendale ed un altra figura per aiutarmi in caso di problematiche durante la configurazione di servizi o durante lo sviluppo.
\begin{figure}[H]
    \centering
...
L'azienda si propone ai suoi nuovi clienti come \textit{Innovation advisor} e grazie a delle sessioni con il cliente, riesce a sviluppare soluzioni innovative in linea con le esigenze del cliente. Essendo \textit{parter} \gls{aws}, l'azienda è particolarmente incentrata nella ricerca di innovazione in ambito \textit{cloud}.
