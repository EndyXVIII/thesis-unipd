...
\subsubsection{UC3: Generazione \gls{embedding-g} del \textit{ticket} completato}
\textbf{Attori principali}: \gls{llm} \\
...
\subsubsection{UC3.1: Salvataggio \textit{ticket} su \textit{database} MongoDB}
\textbf{Attori principali}: Sistema \\
...
\begin{figure}[H]
    ...
    \caption{Diagramma del caso d'uso UC3 e del relativo sottocaso}
    \label{fig:UC3}
\end{figure}
...
\begin{longtable}{|p{1.5cm}|p{5.5cm}|p{2cm}|p{2.5cm}|} 
    ...
    \hline
    \rowcolor{tableevenrow} R-FO-6 & Il sistema deve poter interrogare un \gls{llm} per la generazione dell'\gls{embedding-g} del \textit{ticket} completato per salvarlo nel \textit{database} & Obbligatorio & UC3 (Jira) \\
    \hline
    R-FO-7 & Il sistema deve permettere il salvataggio del \textit{ticket} completato e del relativo \gls{embedding-g} nel \textit{database}. & Obbligatorio & UC3.1 (Jira) \\
    \hline
...
\end{Verbatim}
\vspace{-0.5cm}
\captionof{figure}{\textit{Prompt} per il sistema di proposte di risoluzione Jira}
...
\end{Verbatim}
\vspace{-0.45cm}
\captionof{figure}{\textit{Prompt} per il \textit{chatbot}}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.93\textwidth]{architetturaBenchmark.png}
    \caption{Architettura del \textit{benchmark}}
    \label{fig:benchmarkArchitecture}
\end{figure}
\noindent
Come mostro nell'immagine \ref{fig:benchmarkArchitecture}, il \textit{benchmark} è composto da quattro componenti principali:
\begin{itemize}
...
\end{minted}
\vspace{-0.5cm}
\captionof{figure}{Configurazione delle funzioni \textit{lambda} del sistema Jira}
